...
import { GameObject, ObjectType, LANE_WIDTH, SPAWN_DISTANCE, REMOVE_DISTANCE, GameStatus, THEMES, GameDifficulty } from '../../types';
...
const PLAYER_BOLT_GEO = new THREE.CylinderGeometry(0.05, 0.05, 1.0, 8);
const HOLE_GEO = new THREE.TorusGeometry(1.0, 0.1, 16, 32);
const HOLE_INNER_GEO = new THREE.CircleGeometry(1.0, 32);
...
const SHADOW_LASER_GEO = new THREE.PlaneGeometry(10, 0.2);
...
const PLATFORM_LENGTH = 12.0;
const PLATFORM_GEO = new THREE.BoxGeometry(LANE_WIDTH, PLATFORM_THICKNESS, PLATFORM_LENGTH);
...
export const LevelManager: React.FC = () => {
    const { 
        ...
        laneCount, setDistance, openShop, level, theme, difficulty, customWord,
        restoreLife, distance, setCurrentFloorY
    } = useStore();
    ...
    // Difficulty Multiplier - Scales with level AND distance
    const difficultyLevel = useMemo(() => {
        const distanceFactor = (distance / 400) * 0.15; // Every 400m difficulty increases by 15%
        const levelFactor = (level - 1) * 0.2;
        const diffMult = difficulty === GameDifficulty.EASY ? 0.7 : difficulty === GameDifficulty.HARD ? 1.5 : 1.0;
        return (1 + levelFactor + distanceFactor) * diffMult;
    }, [level, difficulty, distance]);
  ...
  useFrame((state, delta) => {
    ...
    for (const obj of currentObjects) {
        ...
        // Bolt Movement
        if (obj.type === ObjectType.PLAYER_BOLT) {
            moveAmount -= BOLT_SPEED * safeDelta;
        }
        ...
        // Bolt Collision check against obstacles
        if (obj.type === ObjectType.PLAYER_BOLT && obj.active) {
            for (const target of currentObjects) {
                if (target.active && (target.type === ObjectType.OBSTACLE || target.type === ObjectType.ALIEN)) {
                    ...
                    if (dx < 1.0 && dz < 1.5) {
                        ...
                        window.dispatchEvent(new CustomEvent('particle-burst', { 
                           detail: { position: target.position, color: '#ff5500' } 
                        }));
                        hasChanges = true;
                    }
                }
        ...
        if (obj.active) {
            ...
            const zHalf = obj.type === ObjectType.PLATFORM ? PLATFORM_LENGTH / 2 : 1.0;
            const inZZone = (prevZ < playerPos.z + zHalf + 0.5) && (obj.position[2] > playerPos.z - zHalf - 0.5);
            ...
            if (obj.type === ObjectType.SHOP_PORTAL) {
                ...
                // Lasers span all lanes horizontally, so we only check Z and Y
                const zThreshold = 1.0;
                const inZZone = (prevZ < playerPos.z + zThreshold) && (obj.position[2] > playerPos.z - zThreshold);
                
                if (inZZone) {
                    const playerBottom = playerPos.y;
                    const playerTop = playerPos.y + (useStore.getState().isDucking ? 0.8 : 1.8);
                    
                    const laserY = obj.position[1];
                    const laserThickness = 0.2;
                    const laserBottom = laserY - laserThickness;
                    const laserTop = laserY + laserThickness;
                ...
                // STANDARD COLLISION
                     ...
                     const hitWidth = obj.type === ObjectType.PLATFORM ? LANE_WIDTH / 2 + 0.1 : 0.9;
 
                     if (dx < hitWidth) { 
                      
                      // Obstacles, Aliens, Missiles and Platforms damage player
                      const isDamageSource = obj.type === ObjectType.OBSTACLE || obj.type === ObjectType.ALIEN || obj.type === ObjectType.MISSILE;
                      const isPlatform = obj.type === ObjectType.PLATFORM;
                      
                      if (isDamageSource || isPlatform) {
                         ...
                         if (obj.type === ObjectType.OBSTACLE) {
                             ...
                             objBottom = PLATFORM_Y - PLATFORM_THICKNESS / 2;
                             objTop = PLATFORM_Y + PLATFORM_THICKNESS / 2 - 0.2; // 20cm forgiveness to prevent hitting top surface
                         }
                         ...
                         if (isHit) { 
                             window.dispatchEvent(new Event('player-hit'));
                             if (!isPlatform) obj.active = false; 
                             hasChanges = true;
                         ...
                         if (playerPos.y < 0.8) {
                            window.dispatchEvent(new Event('player-fall'));
                            obj.active = false;
                            hasChanges = true;
                         }
                     } else {
                         ...
                         if (dy < 2.5) { // Generous vertical pickup range
                            ...
                            if (obj.type === ObjectType.LIFE) {
                                restoreLife();
                                audio.playGemCollect(); // Reuse gem sound
                            }
    ...
    if (furthestZ > -SPAWN_DISTANCE) {
         ...
         const minGap = (12 + (speed * 0.4)) / difficultyLevel; 
         ...
         if (isLetterDue) {
             ...
             if (availableIndices.length > 0) {
                 ...
                 const color = themeColorList[chosenIndex % themeColorList.length];
                 ...
                 nextLetterDistance.current += getLetterInterval(level) / difficultyLevel;
                 hasChanges = true;
             } else {
            ...
            const isObstacle = Math.random() > (0.20 / difficultyLevel);

            if (isObstacle) {
                // Decide between Alien (Level 2+), Hole, Platform or Spikes
                const randValue = Math.random();
                const spawnPlatform = randValue < 0.15; 
                const spawnAlien = !spawnPlatform && level >= 2 && randValue < (0.35 * difficultyLevel); 
                const spawnHole = !spawnPlatform && !spawnAlien && randValue < (0.55 * difficultyLevel);

                if (spawnPlatform) {
                    const lane = getRandomLane(laneCount);
                    const posZ = spawnZ - PLATFORM_LENGTH / 2;
                    keptObjects.push({
                        id: uuidv4(),
                        type: ObjectType.PLATFORM,
                        position: [lane * LANE_WIDTH, PLATFORM_Y, posZ],
                        active: true,
                    });
                    // Add items on top of platform
                    keptObjects.push({
                        id: uuidv4(),
                        type: ObjectType.GEM,
                        position: [lane * LANE_WIDTH, PLATFORM_Y + 1.2, posZ],
                        active: true,
                        color: colors.accent,
                        points: 100
                    });
                    // Chance for a letter on platform
                    if (Math.random() < 0.3) {
                         const target = customWord.split('');
                         const availableIndices = target.map((_, i) => i).filter(i => !collectedLetters.includes(i));
                         if (availableIndices.length > 0) {
                             ...
                             keptObjects.push({
                                id: uuidv4(),
                                type: ObjectType.LETTER,
                                position: [lane * LANE_WIDTH, PLATFORM_Y + 1.2, posZ - 3], 
                                active: true,
                                color: colors.primary,
                    ...
                    if (pAlien > (0.7 / difficultyLevel)) {
                        alienCount = Math.min(2, availableLanes.length);
                    }
                    if (pAlien > (0.9 / difficultyLevel) && availableLanes.length >= 3) {
                        alienCount = 3;
                    }
                    ...
                    if (p > (0.80 / difficultyLevel)) {
                        countToSpawn = Math.min(3, availableLanes.length);
                    } else if (p > (0.50 / difficultyLevel)) {
            ...
            } else if (Math.random() < (0.15 * difficultyLevel)) { // 15% chance for Laser if obstacle was not chosen
                keptObjects.push({
                    id: uuidv4(),
                    type: ObjectType.LASER,
                    position: [0, Math.random() > 0.5 ? 1.5 : 0.5, spawnZ], // 1.5 = Duck under, 0.5 = Jump over
                    active: true,
                    color: colors.secondary
                });
            } else {
                // GROUND GEM or LIFE SPAWNING
                const lane = getRandomLane(laneCount);
                const spawnLife = Math.random() < 0.05; // 5% chance of life instead of gem
                keptObjects.push({
                    id: uuidv4(),
                    type: spawnLife ? ObjectType.LIFE : ObjectType.GEM,
                    position: [lane * LANE_WIDTH, 1.2, spawnZ],
                    active: true,
                    color: spawnLife ? '#ff0054' : colors.primary,
                    points: 50
                });
            }
            hasChanges = true;
    ...
    // 3. Platform Floor Detection
    let maxFloorY = 0;
    for (const obj of objectsRef.current) {
        if (obj.type === ObjectType.PLATFORM && obj.active) {
            const dx = Math.abs(obj.position[0] - playerPos.x); // PlayerPos.x is the actual world X
            const dz = Math.abs(obj.position[2] - playerPos.z); // PlayerPos.z is where the player is in world space
            
            // Note: Since playerPos is in world space and objects are in relative space (but moving),
            // this check is correct if playerPos.z is compared accurately.
            // Actually, in this world, player is stationary at Z and world moves? 
            // Let's re-verify:
            // "obj.position[2] += moveAmount;" where moveAmount is distance world moves.
            // Player is usually at some fixed Z.
            
            if (dx < LANE_WIDTH / 2 + 0.1 && dz < PLATFORM_LENGTH / 2) {
                if (playerPos.y >= PLATFORM_Y - 0.5) {
                    maxFloorY = Math.max(maxFloorY, PLATFORM_Y + PLATFORM_THICKNESS / 2);
                }
            }
        }
    }
    setCurrentFloorY(maxFloorY);
  });
...
const GameEntity: React.FC<{ data: GameObject }> = React.memo(({ data }) => {
    ...
    const { laneCount, distance } = useStore();
    
    const scale = useMemo(() => {
        if (data.type === ObjectType.OBSTACLE) {
            return Math.min(1 + (distance / 4000), 1.6);
        }
        return 1;
    }, [data.type, distance]);
    ...
    const shadowGeo = useMemo(() => {
        ...
        if (data.type === ObjectType.GEM || data.type === ObjectType.LIFE) return SHADOW_GEM_GEO;
        ...
        if (data.type === ObjectType.LASER) return SHADOW_LASER_GEO;
        if (data.type === ObjectType.HOLE) return null;
        if (data.type === ObjectType.PLAYER_BOLT) return SHADOW_GEM_GEO; // repurpose small circle
    ...
    return (
        <group ref={groupRef} position={[data.position[0], 0, data.position[2]]}>
            ...
            {data.type === ObjectType.LASER && (
                <group>
                    <mesh position={[-(laneCount * LANE_WIDTH + 2) / 2, 5, 0]}>
            ...
            <group ref={visualRef} position={[0, data.position[1], 0]} scale={[scale, scale, scale]}>
                {/* --- SHOP PORTAL --- */}
                ...
                {data.type === ObjectType.LASER && (
                    <group>
                        <mesh rotation={[0, 0, Math.PI / 2]} scale={[1, laneCount * LANE_WIDTH + 2, 1]}>
                            <cylinderGeometry args={[0.08, 0.08, 1, 8]} />
                            <meshStandardMaterial color={data.color} emissive={data.color} emissiveIntensity={5} />
                        </mesh>
                        {/* Glow Beam */}
                        <mesh rotation={[0, 0, Math.PI / 2]} scale={[1, laneCount * LANE_WIDTH + 2, 1]}>
                ...
                {/* --- LIFE --- */}
                {data.type === ObjectType.LIFE && (
                    <group scale={[0.8, 0.8, 0.8]}>
                        <mesh rotation={[0, 0, Math.PI]} position={[0, 0.2, 0]}>
                ...
                {/* --- HOLE --- */}
                {data.type === ObjectType.HOLE && (
                    <group rotation={[-Math.PI / 2, 0, 0]}>
                        <mesh geometry={HOLE_GEO}>
                            <meshBasicMaterial color="#00ffff" />
                        </mesh>
                        <mesh geometry={HOLE_INNER_GEO} position={[0, 0, -0.05]}>
                            <meshBasicMaterial color="#000000" />
                        </mesh>
                    </group>
                )}
                ...
                {data.type === ObjectType.PLATFORM && (
                    <group>
                        <mesh geometry={PLATFORM_GEO} receiveShadow>
                            <meshStandardMaterial 
                                color="#111111" 
                                metalness={0.9} 
                                roughness={0.1}
                        ...
                        <mesh position={[LANE_WIDTH/2, 0, 0]}>
                            <boxGeometry args={[0.1, PLATFORM_THICKNESS + 0.05, PLATFORM_LENGTH]} />
                            <meshBasicMaterial color="#00ffff" />
                        </mesh>
                        <mesh position={[-LANE_WIDTH/2, 0, 0]}>
                            <boxGeometry args={[0.1, PLATFORM_THICKNESS + 0.05, PLATFORM_LENGTH]} />
                ...
                {/* --- PLAYER BOLT --- */}
                {data.type === ObjectType.PLAYER_BOLT && (
                    <group rotation={[Math.PI / 2, 0, 0]}>
                        <mesh geometry={PLAYER_BOLT_GEO}>
