...
import { LANE_WIDTH, THEMES } from '../../types';
...
const LaneGuides: React.FC = () => {
    const { laneCount, theme } = useStore();
...
const RetroSun: React.FC = () => {
    ...
    const uniforms = useMemo(() => ({
        uTime: { value: 0 },
        uColorTop: { value: new THREE.Color(colors.accent) }, 
        uColorBottom: { value: new THREE.Color(colors.secondary) } 
    }), [colors]);
...
export const Environment: React.FC = () => {
  ...
  return (
    <>
      ...
      <ambientLight intensity={0.2} color={colors.secondary} />
      <directionalLight position={[0, 20, -10]} intensity={1.5} color={colors.primary} />
      <pointLight position={[0, 25, -150]} intensity={2} color={colors.accent} distance={200} decay={2} />
