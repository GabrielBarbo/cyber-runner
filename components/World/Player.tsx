...
const CYBER_TORSO_GEO = new THREE.BoxGeometry(0.4, 0.7, 0.3);
const ALIEN_TORSO_GEO = new THREE.SphereGeometry(0.35, 8, 8);
...
const ALIEN_HEAD_GEO = new THREE.IcosahedronGeometry(0.25, 1);
...
export const Player: React.FC = () => {
  ...
  const { status, laneCount, takeDamage, hasDoubleJump, activateImmortality, isImmortalityActive, isDucking, setIsDucking, avatarStyle, hasBlaster, currentFloorY } = useStore();
  ...
  const { armorMaterial, jointMaterial, glowMaterial, shadowMaterial } = useMemo(() => {
      ...
      if (avatarStyle === AvatarStyle.CYBERPUNK) {
          armorColor = '#ff00aa';
          glowColor = '#00ffaa';
      } else if (avatarStyle === AvatarStyle.ALIEN) {
          armorColor = '#00ff00';
          glowColor = '#ffff00';
          metalness = 0.4;
          roughness = 0.1;
      ...
      return {
          armorMaterial: new THREE.MeshStandardMaterial({ color: armorColor, roughness, metalness }),
  ...
  }, [isImmortalityActive, avatarStyle]); 
  ...
  const triggerDuck = () => {
    if (isJumping.current || isDucking) return;
    setIsDucking(true);
    setTimeout(() => {
        setIsDucking(false);
    }, 800); // Duck duration
  ...
  const triggerShoot = () => {
    if (!hasBlaster || status !== GameStatus.PLAYING) return;
    window.dispatchEvent(new CustomEvent('player-shoot', {
        detail: {
            lane: lane,
            x: groupRef.current?.position.x,
            y: groupRef.current?.position.y + 1.1,
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      else if (e.key === 'ArrowDown' || e.key === 's') triggerDuck();
      else if (e.key === 'f' || e.key === 'q') triggerShoot();
  ...
  useEffect(() => {
    ...
    window.addEventListener('player-fall', handleFall);
    return () => window.removeEventListener('player-fall', handleFall);
  }, [isImmortalityActive]);
  ...
  useFrame((state, delta) => {
    ...
    // Falling Effect
    if (isFalling) {
        groupRef.current.position.y -= delta * 15;
    ...
    // 2. Physics (Jump)
    if (!isJumping.current && Math.abs(groupRef.current.position.y - currentFloorY) > 0.01) {
        if (groupRef.current.position.y > currentFloorY) {
            // Fall off platform
            isJumping.current = true;
            jumpsPerformed.current = 0; // Allow a jump while falling?
            velocityY.current = 0;
        } else {
            // Floor rose up (e.g. platform started)
            groupRef.current.position.y = currentFloorY;
        }
    }
    ...
    if (isDucking) {
        // Ducking Pose (Crouch)
        const duckSpeed = delta * 15;
        if (bodyRef.current) bodyRef.current.position.y = THREE.MathUtils.lerp(bodyRef.current.position.y, 0.6, duckSpeed);
        if (leftArmRef.current) leftArmRef.current.rotation.x = THREE.MathUtils.lerp(leftArmRef.current.rotation.x, -1.0, duckSpeed);
        if (rightArmRef.current) rightArmRef.current.rotation.x = THREE.MathUtils.lerp(rightArmRef.current.rotation.x, -1.0, duckSpeed);
        if (leftLegRef.current) leftLegRef.current.rotation.x = THREE.MathUtils.lerp(leftLegRef.current.rotation.x, -1.5, duckSpeed);
        if (rightLegRef.current) rightLegRef.current.rotation.x = THREE.MathUtils.lerp(rightLegRef.current.rotation.x, -1.5, duckSpeed);
    ...
    <group ref={groupRef} position={[0, 0, 0]}>
      <group ref={bodyRef} position={[0, 1.1, 0]}> 
        ...
        <mesh castShadow position={[0, 0.2, 0]} 
            geometry={avatarStyle === AvatarStyle.ROBOT ? TORSO_GEO : avatarStyle === AvatarStyle.CYBERPUNK ? CYBER_TORSO_GEO : ALIEN_TORSO_GEO} 
            material={armorMaterial} 
        />

        {/* Jetpack */}
        {avatarStyle !== AvatarStyle.ALIEN && (
            <>
        ...
        <group ref={headRef} position={[0, 0.6, 0]}>
            <mesh castShadow geometry={avatarStyle === AvatarStyle.ALIEN ? ALIEN_HEAD_GEO : HEAD_GEO} material={armorMaterial} />
        </group>
